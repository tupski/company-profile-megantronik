        ...
        <div class="container py-4">
            <div class="row justify-content-center">
                <div class="col-lg-8 text-center">
                    <h1 class="display-5 fw-bold mb-3">Kategori: {{ $category->name }}</h1>
                            ...
                            <ul class="list-group list-group-flush">
                                @foreach($categories as $cat)
                                    <li class="list-group-item d-flex justify-content-between align-items-center {{ $cat->id == $category->id ? 'active bg-light' : '' }}">
                                        <a href="{{ route('blog.category', $cat->slug) }}" class="text-decoration-none {{ $cat->id == $category->id ? 'text-primary fw-bold' : 'text-dark' }}">
                                            {{ $cat->name }}
                                        </a>
                                        <span class="badge {{ $cat->id == $category->id ? 'bg-primary' : 'bg-secondary' }} rounded-pill">{{ $cat->posts_count }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
